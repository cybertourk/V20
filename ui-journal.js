...
export function updateJournalList(newData) {
    ...
    // Only refresh list if it exists
...
function renderJournalInterface(container, activeTab) {
    ...
    if (activeContext.mode === 'player') {
        ...
        const btnSessions = document.createElement('button');
        btnSessions.className = `text-xs uppercase tracking-wider px-2 pb-1 ${activeTab==='sessions' ? 'border-b-2 border-[#d4af37] text-[#d4af37] font-bold' : 'text-gray-500 hover:text-white transition-colors'}`;
        btnSessions.innerText = "Session Logs";
        btnSessions.onclick = () => {
    ...
    autocomplete.className = "autocomplete-box";
    wrapper.appendChild(autocomplete);

    container.appendChild(wrapper);

    // Injects Modals if missing
    ensureModalsExist();
    ...
    if (!document.getElementById('recipient-modal')) {
        ...
        rm.innerHTML = `
            <div class="bg-[#1a1a1a] border border-[#d4af37] p-6 max-w-sm w-full shadow-2xl relative">
                ...
                <div id="recipient-list" class="space-y-1 max-h-48 overflow-y-auto mb-4 custom-scrollbar border border-[#333] bg-[#050505] p-2 rounded"></div>
...
// ==========================================================================
// VIEW 1: SESSION LOGS (Rebuilt DOM API)
// ==========================================================================
...
// --- SESSION FORM RENDERER (Heavy DOM) ---
...
function addSceneRow(container, data, idx) {
    ...
    if (idx > 0) div.querySelector('.remove-scene').onclick = function() { div.remove(); };
}
...
function addNpcRow(container, data) {
    ...
    if (!data.attitude) radios[1].checked = true; // Default Neutral
...
// --- GLOBAL HELPERS FOR BUTTONS (Now proxies to DOM logic) ---

window.initNewSessionLog = initNewSessionLog;
window.deleteSessionLog = deleteSessionLog;
window.addLogScene = (cont) => addSceneRow(document.getElementById('container-scenes') || cont, {id:0, text:''}, document.querySelectorAll('.scene-row').length);
window.addLogClue = (cont) => addClueRow(document.getElementById('container-investigation') || cont, {id:0, text:''}, document.querySelectorAll('.clue-row').length);
...
function saveSessionLog(id, isNew) {
    ...
    const updated = {
        ...
        status: { 
            ...
            health: "Healthy", // Simplify for now or read boxes
...
function renderCodexView(container) {
    ...
    main.className = "w-3/4 h-full bg-[#080808] border border-[#333] p-6 relative overflow-y-auto custom-scrollbar no-scrollbar";
    main.id = "codex-editor";
    main.classList.add('hidden'); // Start hidden
    
    // Inject Editor HTML via String (It's static structure)
    main.innerHTML = `
    ...
    const btnUrl = main.querySelector('#cx-btn-url');
    const inputUpload = main.querySelector('#cx-file');
    const uploadBtn = main.querySelector('#cx-upload-btn');
    const btnRemove = main.querySelector('#cx-remove-img');
    const preview = main.querySelector('#cx-img-preview');
...
function bindPopupListeners() {
    // Only bind if elements exist (Modals might be dynamic)
