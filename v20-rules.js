            ...
            if (window.state.xpMode && clickVal > currentVal) {
                ...
                for (let v = currentVal + 1; v <= clickVal; v++) {
                    if (type === 'abil' || isAbil) {
                        ...
                        // Backgrounds usually cost 0 XP in V20 core, unless house ruled.
                        totalCost += 0; 
                        costDesc = "Background";
                    } else if (type === 'disc') {
                        // Disciplines: New=10, Raise=x5 (Clan) or x7 (Out), x6 (Caitiff)
                        if (v === 1) {
                            totalCost += 10;
                        } else {
                            let mult = 7;
                            if (clan === "Caitiff") mult = 6;
                            else {
                                // Simple In-Clan check based on known Clan Disciplines would be ideal here.
                                // For now, we assume user knows what they are doing or default to Out-of-Clan (7).
                                // NOTE: In a full app, we would check against a list of In-Clan disciplines.
                                // We'll default to 7 here as it's safer, user can adjust if needed or we can add clan check later.
                                // If the name matches standard clan disciplines, we could try to detect it.
                                // Let's use 7 as default.
                                mult = 7; 
                                // Attempt basic detection if possible (future enhancement)
                            }
                            totalCost += (v - 1) * mult;
                        }
                        costDesc = "Discipline";
