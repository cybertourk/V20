...
window.showNotification = function(msg) { 
    ...
    if (el) { 
        ...
        // Reset color in case it was red from error
            ...
            if (specs.length > 0) {
                 ...
                 if(isApplied) {
                     ...
                     if(btn) btn.onclick = (e) => {
                         ...
                         if(cb) { cb.checked = true; window.showNotification(`Applied: ${specs.join(', ')}`); hintEl.innerHTML = `<span class="text-[#d4af37] font-bold">Specialty Active! (10s = 2 Successes)</span>`; }
                     };
                 }
...
// --- CORE UI UPDATE (The most important function) ---
...
window.updatePools = function() {
    ...
    // Sync Permanent ratings in Creation Mode
...
window.setDots = function(name, type, val, min, max = 5) {
    ...
    // Auto-update Humanity/Willpower in creation
...
window.renderDynamicAdvantageRow = function(containerId, type, list, isAbil = false) {
    ...
    const buildRow = (name = "") => {
        ...
        dotCont.onclick = (e) => { 
            ...
            // Reuse setDots logic via direct modification to avoid complications
            window.setDots(curName, type, parseInt(e.target.dataset.v), 0, 5);
            // Re-render this specific row to capture specialty state changes
            const newV = window.state.dots[type][curName];
            if ((newV >= 1 && val === 0) || (newV === 0 && val >= 1)) {
                 // Simple refresh: Clear wrapper and rebuild based on new state
...
window.renderDerangementsList = function() {
    ...
    window.state.derangements.forEach((d, idx) => {
        ...
        row.innerHTML = `<span>${d}</span><span class="remove-btn text-red-500" onclick="window.state.derangements.splice(${idx}, 1); window.renderDerangementsList(); window.updatePools();">&times;</span>`;
        cont.appendChild(row);
    });
...
window.fullRefresh = function() {
    window.hydrateInputs();
    
    // Lists
    const mList = document.getElementById('merits-list-create');
    if(mList) { mList.innerHTML = ''; window.renderDynamicTraitRow('merits-list-create', 'Merit', V20_MERITS_LIST); }
    const fList = document.getElementById('flaws-list-create');
    if(fList) { fList.innerHTML = ''; window.renderDynamicTraitRow('flaws-list-create', 'Flaw', V20_FLAWS_LIST); }
    ...
    if(window.state.dots.abil) Object.keys(ABILITIES).forEach(c => ABILITIES[c].forEach(a => { window.refreshTraitRow(a, 'abil'); }));
