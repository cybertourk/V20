...
export async function renderChronicleTab() {
    ...
    try {
        ...
        document.getElementById('tab-chron-roster').onclick = () => {
            window.state.chronicleSubTab = 'roster';
            renderChronicleTab(); 
        };
        ...
        if (currentTab === 'info') {
            ...
            renderChronicleRosterView(contentArea);
        } else {
...
function renderChronicleRosterView(container) {
    // Filter players, removing system objects
    const players = Object.values(stState.players || {})
        .filter(p => !p.metadataType || p.metadataType !== 'journal');

    if (players.length === 0) {
        container.innerHTML = `<div class="text-center text-gray-500 mt-10 italic">No other kindred found in the shadows...</div>`;
        return;
    }

    let html = `<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 overflow-y-auto h-full custom-scrollbar pb-20">`;
    
    players.forEach(p => {
        const sheet = p.full_sheet || {};
        const name = p.character_name || "Unknown";
        const clan = sheet.textFields?.['c-clan'] || "Unknown Clan";
        const concept = sheet.textFields?.['c-concept'] || "";
        const img = sheet.characterImage || null;
        
        let imgHtml = `<div class="w-full h-32 bg-[#0a0a0a] flex items-center justify-center text-gray-700 border-b border-[#333]"><i class="fas fa-user text-4xl opacity-50"></i></div>`;
        if (img) {
             const src = window.convertGoogleDriveLink ? window.convertGoogleDriveLink(img) : img;
             imgHtml = `<div class="w-full h-32 bg-cover bg-center bg-no-repeat border-b border-[#333]" style="background-image: url('${src}')"></div>`;
        }

        const isOnline = p.status !== 'Offline';
        const statusDot = isOnline ? `<span class="w-2 h-2 rounded-full bg-green-500 inline-block mr-1"></span>` : `<span class="w-2 h-2 rounded-full bg-red-900 inline-block mr-1"></span>`;

        html += `
            <div class="bg-[#111] border border-[#333] flex flex-col shadow-lg group hover:border-[#d4af37] transition-all overflow-hidden">
                ${imgHtml}
                <div class="p-3 flex-1 flex flex-col">
                    <div class="text-[#d4af37] font-bold text-sm leading-tight mb-1 truncate font-cinzel">${name}</div>
                    <div class="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-2">${clan}</div>
                    ${concept ? `<div class="text-[10px] text-gray-400 italic leading-snug line-clamp-3 mb-2 flex-1">"${concept}"</div>` : '<div class="flex-1"></div>'}
                    <div class="mt-2 pt-2 border-t border-[#222] text-[9px] text-gray-600 flex items-center">
                         ${statusDot} <span class="uppercase font-bold">${isOnline ? 'Awake' : 'Torpor'}</span>
                    </div>
                </div>
            </div>
        `;
